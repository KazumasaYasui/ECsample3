class Order < ApplicationRecord
  ...
  with_options on: :order_complete do |order_complete|
    order_complete.validates :cust_name, presence: true,
                                         length: { maximum: 100 }
    order_complete.validates :cust_email, presence: true,
                                          length: { maximum: 100 }
    order_complete.validates :cust_tel, presence: true,
                                        format: {
                                          with: /\A\d+-\d+-\d+\z/
                                        }
    order_complete.validates :cust_postal_code, presence: true,
                                                format: {
                                                  with: /\A\d{7}\z/
                                                }
    order_complete.validates :cust_prefecture, presence: true
    order_complete.validates :cust_city, presence: true,
                                         length: { maximum: 100 }
    order_complete.validates :cust_street, presence: true,
                                           length: { maximum: 100 }
  end
